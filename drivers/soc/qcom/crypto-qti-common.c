...
struct ice_part_cfg {
	...
	char	volname[PARTITION_META_INFO_VOLNAMELTH + 1];
	uint32_t	key_slot;
	uint32_t	add_partition_result;
	bool	encr_bypass;
	bool	decr_bypass;
	struct kobject	kobj;
...
/**
 * crypto_qti_ice_add_new_partition() - Add a new partition for the ICE to manage
 *
				   ...
				   size_t count)
{
	...
	list_for_each(new_pos, &ice_dev->part_cfg_list) {
		...
		if (!strcmp(label, elem->volname)) {
			dev_err(dev, "partition exist already, needn't to add\n");
	...
	if (key_res) {
		if (key_res == QSEECOM_KEY_ID_EXISTS) {
			...
			--ice_dev->num_fde_slots_in_use; /* Take the slot back */
			ret = -EINVAL;
			goto out;
		}
	}
					 ...
					 bool decr_bypass)
{
	...
	if ((short)(crypto_data->key_index) >= 0) {
		...
		if (rq_data_dir(req) == WRITE) {
			...
			/*
			 * Non READ/WRITE requests do not require ICE programming.
			 * Keep ICE in bypass mode and allow the request to proceed.
			 */
			pr_debug("%s unhandled request 0x%x\n", __func__, req->cmd_flags);
					   ...
					   struct ice_device *ice_dev)
{
	...
	if (cnt <= 0) {
		dev_err(dev, "%s: Unable to find clocks, assuming enabled\n", __func__);
		ret = cnt;
		goto out;
	}

	if (!of_get_property(np, "qcom,op-freq-hz", &len)) {
		dev_err(dev, "qcom,op-freq-hz property not specified\n");
		goto out;
	}

	len = len / sizeof(*clkfreq);
	if (len != cnt) {
		dev_err(dev, "%s: clock-names count %d != qcom,op-freq-hz count %d\n",
			__func__, cnt, len);
		ret = -EINVAL;
		goto out;
	}
	...
	if (ret) {
		dev_err(dev, "%s: failed to read qcom,op-freq-hz, ret=%d\n",
			__func__, ret);
		goto out;
	}
				       ...
				       struct ice_device *ice_dev)
{
	...
	if (!of_parse_phandle(pdev->dev.of_node, "vdd-hba-supply", 0)) {
		pr_debug("%s: No vdd-hba-supply regulator, assuming not needed\n", __func__);
...
/**
 * crypto_qti_ice_cleanup_sysfs() - Remove all sysfs resources associated with the driver
 *
 * @ice_dev:	ICE device node
 */
